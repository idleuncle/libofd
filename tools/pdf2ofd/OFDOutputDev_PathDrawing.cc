...
cairo_matrix_t GfxCTM_to_CairoMatrix(GfxState *state){
...
void OFDOutputDev::stroke(GfxState *state) {
  ...
  //}
    ...
    if ( ofdPath != nullptr ){
        ...
        //matrix1.x0 = gfxCTM[4];
        //matrix1.y0 = gfxCTM[5];
        //cairo_matrix_t matrix1 = GfxCTM_to_CairoMatrix(state);
...
PathObjectPtr OFDOutputDev::createPathObject(GfxState *state){
    ...
    if ( ofdPath != nullptr ){
        ...
        // FIXME use m_clipPath instead of ofdPath.
        if ( m_clipPath != nullptr ){
            ofdPath = m_clipPath;
            //ofdPath->Append(m_clipPath);
        }
        ...
        if ( m_clipPath != nullptr ){
            ...
            fillColor = GfxColor_to_OfdColor(&m_fillColor);
        }
...
void OFDOutputDev::fill(GfxState *state) {
    ...
    if ( drawState.Debug.Enabled && drawState.Debug.PathObjectID != 0 && drawState.Debug.PathObjectID != pathObject->ID ){
        return;
    }
...
void OFDOutputDev::clip(GfxState *state) {
    ...
    if ( m_numSaveState == 2 ){
        if ( !( clipPath->GetNumSubpaths() == 1 && clipPath->GetSubpath(0)->GetNumPoints() <= 5 )){
            if ( m_clipPath == nullptr ){
                m_clipPath = clipPath;
                m_clipMatrix = GfxCTM_to_CairoMatrix(state);
                state->getFillRGB(&m_clipFillColor);
            } else {
    ...
    //{
    //std::stringstream ssCairoLog;
    //ssCairoLog << "[imageSurface] my CTM = (" << m_matrix.xx << ", " << m_matrix.yx << ", " << m_matrix.xy 
        //<< ", " << m_matrix.yy << ", " << m_matrix.x0 << ", " << m_matrix.y0 << ")";
    //std::string cairoLog = ssCairoLog.str() + "\n";
    //cairoLogFile.write(cairoLog.c_str(), cairoLog.length());
    //}
    ...
    //ssCairoLog << "[imageSurface] state CTM = (" << ctm[0] << ", " << ctm[1] << ", " << ctm[2] 
        //<< ", " << ctm[3] << ", " << ctm[4] << ", " << ctm[5] << ")";
    //std::string cairoLog = ssCairoLog.str() + "\n";
    //cairoLogFile.write(cairoLog.c_str(), cairoLog.length());
    //}
    ...
    //{
    //std::stringstream ssCairoLog;
    //std::string objectString = clipPath1->to_string();
    //ssCairoLog << "[imageSurface] clip1():\n" << objectString;
    //std::string cairoLog = ssCairoLog.str();
    //cairoLogFile.write(cairoLog.c_str(), cairoLog.length());
    //}
